        ...
        try {
            ...
            reader.onloadend = async function () {
                ...
                const conversationPromises = conversations.map(conv =>
                    getConversationByOrganizationIdAndById(selectOrganizationId, conv.id)
                        .catch(error => {
                            console.error(`Error al obtener detalles de conversación ${conv.id}:`, error);
                            return null;
                        })
                );
                ...
                conversationDetails.forEach((conversationDetail, index) => {
                    ...
                    if (messages.length > 0) {
                        ...
                        for (const message of messages) {
                            ...
                            const hasImage = (message.images && message.images.length > 0) || messageText.includes('<img') || messageText.includes('data:image');
                            const displayText = messageText || (hasImage ? '[Imagen adjunta]' : '');
                            
                            docDefinition.content.push({
                                columns: [
                                    {
                                        width: 'auto',
                                        text: message.type === MessageType.USER ? 'Usuario: ' : 'Asistente: ',
                                        bold: true,
                                        marginRight: 5
                                    },
                                    {
